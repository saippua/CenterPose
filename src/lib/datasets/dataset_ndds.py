...
def filter(files, pallet_id, threshold): 
    ret = []
    for noext in tqdm(files):
        if not os.path.isfile(f"{noext}.cs.png"):
            print(f"Could not verify visiblity. No segmentation mask found! {noext}.cs.png")
            ret.append(noext)
            continue
        with Image.open(f"{noext}.cs.png") as mask:
            vals, counts = np.unique(np.array(mask), return_counts=True)
            if pallet_id not in vals:
                mask.show()
                continue

            ind = np.where(vals==pallet_id)[0][0]
            ind = list(vals).index(pallet_id)
            if counts[ind] < threshold:
                mask.show()
                continue
